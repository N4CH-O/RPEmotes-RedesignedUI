...
function ShowPedMenu(zoom)
    if not Config.PreviewPed or not PreviewEnabled then return end
    ...
    if not ShowPed then
        CreateThread(function()
            ...
            while ShowPed do
                -- Ped is next to the menu (approx 62% from the left if menu is right)
                local screencoordsX = zoom and 0.45 or 0.62
                local screencoordsY = zoom and 1.9 or 0.77

                -- Mirror the position if the menu is on the left
                if Config.MenuPosition == "left" then
                    screencoordsX = 1.0 - screencoordsX -- results in 0.38
                end

                local world, normal = GetWorldCoordFromScreenCoord(screencoordsX, screencoordsY)
                local depth = zoom and 2.0 or 4.5
