...
local function CrouchKeyPressed()
    DebugPrint("CrouchKeyPressed triggered")
    if LocalPlayer.state.canEmote == false then return end
    ...
    if isCrouched then
        isCrouched = false
        local crouchKey = GetControlInstructionalButton(0, `+rp_crouch` | 0x80000000, false)
    ...
    if crouchKey == duckKey then
        if Config.CrouchOverrideStealthMode then
            -- Immediately disable the default duck to prevent stealth mode "flicker"
            DisableControlAction(0, 36, true)
            DisableControlUntilReleased(0, 36)
...
if Config.CrouchEnabled then
    if Config.CrouchKeybindEnabled then
        RegisterKeyMapping('+rp_crouch', Translate('register_crouch'), 'keyboard', Config.CrouchKeybind)
        RegisterCommand('+rp_crouch', function() CrouchKeyPressed() end, false)
        RegisterCommand('-rp_crouch', function() end, false) -- This needs to be here to prevent warnings in chat
    end
